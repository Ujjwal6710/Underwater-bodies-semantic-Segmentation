...
if __name__ == '__main__':
    ...
    for i, filename in enumerate(in_files):
        # Ensure model is loaded correctly before predictions
        logging.info(f'Predicting image {filename} ...')

        try:
            img = Image.open(filename).convert("RGB")
            mask = predict_img(model=model, full_img=img, scale_factor=args.scale, out_threshold=args.mask_threshold)

            if not args.no_save:
                out_filename = out_files[i]  # This is the output filename provided by the user

                # Ensure the output directory exists
                os.makedirs(os.path.dirname(out_filename), exist_ok=True)

                # Generate and save the mixed overlap image using the same name
                create_mixed_overlap(img, mask, out_filename)
                logging.info(f'Mixed overlap saved to {out_filename}')
