...
from tensorflow.keras.utils import img_to_array, load_img, plot_model  # Add plot_model for model visualization
...
def create_mixed_overlap(original, mask, output_path):
    """Create an image showing the original, mask, and mixed overlap."""
    ...
    plt.close()
    logging.info(f'Mixed overlap saved to {output_path}')

def get_args():
    parser = argparse.ArgumentParser(description='Predict masks from input images using DeepLabV3')
    parser.add_argument('--model', '-m', default='saved_models/deeplab.pkl', metavar='FILE',
...
def save_model_image(model, output_dir):
    """Save the model architecture image."""
    model_image_path = os.path.join(output_dir, "model_architecture.png")
    plot_model(model, to_file=model_image_path, show_shapes=True, show_layer_names=True)
    logging.info(f'Model architecture image saved to {model_image_path}')
...
if __name__ == '__main__':
    ...
    # Save the model architecture image
    save_model_image(model, output_dir)
    ...
    for i, filename in enumerate(in_files):
        ...
        try:
            ...
            if not args.no_save:
                out_filename = os.path.join(output_dir, os.path.basename(out_files[i]))
                result = mask_to_image(mask)
                result.save(out_filename)
                logging.info(f'Mask saved to {out_filename}')

                # Generate mixed overlap visualization in output_predict folder
                mixed_overlap_path = os.path.join(output_dir, f'{os.path.splitext(os.path.basename(out_filename))[0]}_mixed.png')
